...
class Simple_Local_Avatars {
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = '' ) {
		...
		$avatar = "<span><a class='fancybox fancybox.iframe' href=' {$url}/user_profile/?usr={$user_id} '></a><img class=\"img-circle\" src='". esc_url( $local_avatars[$size] ) . "'></span>";
	...
	<label for="simple-local-avatar" class=" ph">
		<?php
	...
	?>
					<div class="form-group">
						<p class="choose-file">
							...
							<p class="remove help-block"><?php if ( current_user_can( 'upload_files' ) && did_action( 'wp_enqueue_media' ) ) : ?><a href="#" class="button hide-if-no-js" id="simple-local-avatar-media"><?php _e( 'Choose from Media Library', 'simple-local-avatars' ); ?></a> &nbsp;<?php endif; ?>
							<a href="<?php echo $remove_url; ?>" class="button item-delete submitdelete deletion" id="simple-local-avatar-remove"<?php if ( empty( $profileuser->simple_local_avatar ) ) echo ''; ?>><?php _e('選択した画像を削除する','simple-local-avatars'); ?></a></p>
						</p>
					<?php
						} else {
...
if ( ! function_exists( 'get_avatar' ) && ( $simple_local_avatars_options = get_option('simple_local_avatars') ) && ! empty( $simple_local_avatars_options['only'] ) ) :
	...
	function get_avatar( $id_or_email, $size = 96, $default = '', $alt = '' ) {
		...
		if ( ! $avatar = $simple_local_avatars->get_avatar( '', $id_or_email, $size, $default, $alt ) ) :
			...
			//$avatar = "<img alt='{$safe_alt}' src='" . $default . "' class='avatar avatar-{$size} photo avatar-default' height='{$size}' width='{$size}' />"; //Duong change on 12/03
            $avatar = "<span><a class='fancybox fancybox.iframe' href=' {$url}/user_profile/?usr={$user_id} '></a><img class=\"img-circle\" src='". $default . "'></span>";
		endif;
