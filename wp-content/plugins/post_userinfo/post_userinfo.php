	...
	<div class="container">
		...
		<?php if ( !get_user_meta($current_user->ID, 'oa_social_login_user_thumbnail', true)) { ?>
			...
			<p id="description_form">
				...
				<b class="text-danger">※確認のため、同じパスワードをもう一度入力してください</b>
		...
		<?php
			if (get_user_meta($current_user->ID, 'oa_social_login_user_thumbnail', true)) { ?>
				<h2>グループランチに参加しますか？<small class="text-danger">※必須</small></h2>
				<p><a href="/quartet/" target="_blank">グループランチとは</a>
				<div id="group_form">
							<?php
				// 4人組制度
		...
		<?php }?>
			...
			<p>登録した情報は、エンジニアサポートCROSS 2014告知サイト・エンジニアサポートCROSS 2014イベントのみで使用されます。<br><a href="<?php bloginfo('url'); ?>/policy/" target="_blank">個人情報保護ポリシーについて</a></p>
		</form>
	</div>
</section>
<?php
...
if(
	...
	if ($_POST['age'] && $_POST['job'] != 1 && $_POST['tags'] && $_POST['group'] != 1) {
		global $wpdb;
...
<section>
	<div class="container registered">
		...
		<div>
			...
			if (get_user_meta($current_user->ID, 'oa_social_login_user_thumbnail', true)) {?>
			<h2>グループランチへの参加</h2>
			<p><a href="/quartet/" target="_blank">グループランチとは</a><br /><?php echo $groups[get_user_meta($current_user->ID, 'group', true)]; ?></p>
			<?php } ?>
			<h2>興味のあるジャンル</h2>
			<?php
		echo '<p>';
		...
		?>
			...
			?>
				<a href="https://twitter.com/share" class="twitter-share-button" data-url="http://www.cross-party.com/" data-text="<?php echo $text ?>" data-lang="ja" data-size="large" data-count="none" data-hashtags=" CROSS2014">Tweet</a> 
				<script>
			<!--
			!function(d,s,id) {
				var js
			...
			<hr />
			<div>
				<p><small>登録した情報は、エンジニアサポートCROSS 2014告知サイト・エンジニアサポートCROSS 2014イベントのみで使用されます。</small>
			...
			<p class="text-right"><a href="/" target="_parent">エンジニアサポートCROSS 2014に戻る</a></p>
		</div>
	</div>
</section>
<?php
...
if($register_in_progress) {
	...
	if ( get_user_meta($user_id, 'avatarType', true)) {
		 echo '<span class="prof-ph img-circle"><img src="'. get_user_meta($user_id, 'oa_social_login_user_thumbnail', true). '"></span>';
	} else {
