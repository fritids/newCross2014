...
function display_form() {
  ...
  ?>
<header class="pkg">
  <p class="headTxt">ようこそ<b><span class="name"><?php echo $current_user->nickname;?></span></b> さん
</header>
<div id="contents">
<h1>ユーザー情報を登録</h1>
...
<p>登録した情報は、エンジニアサポートCROSS 2013告知サイト・エンジニアサポートCROSS 2013イベントのみで使用されます。<br><a href="<?php bloginfo('url'); ?>/policy/" target="_blank">個人情報保護ポリシーについて</a></p>
  ...
  <h2>Password</h2>
    <div id="description_form">
      ...
      <input name="repassword" value="" type="password"></input>
    </div>
...
　 <h2>アバター変更</h2>　
  ...
  if (get_user_meta($current_user->ID, 'oa_social_login_user_thumbnail', true)) {
      echo '<input type="checkbox" name="twitterAvatar" value="yes" checked>Twitter avatarで代表画像にしたい';
  }?>
  <?php
...
function show_avatar($user_id) {
  ...
  if (get_user_meta($user_id, 'oa_social_login_user_thumbnail', true)) {
    ...
    if ( get_user_meta($user_id, 'avatarType', true)) {
       echo '<p class="ph"><img src="'. get_user_meta($user_id, 'oa_social_login_user_thumbnail', true). '">test</p>';
    } else {
